...
const page = async ({ params }: { params: { id: string } }) => {
  ...
  const fromDirecting = detailWithExtra.credits.crew.filter(member => member.department === 'Directing')?.map(member => ({ name: member.name, jobs: detailWithExtra.credits.crew.filter(m => member.name === m.name).map(m => m.job).join(`, `)  }))
    ...
    <>
      <section className="w-full py-7 relative">
        ...
        >
          ...
          <div className="py-6">
          <header>
          <h1>{detail.title} {detail.release_date}</h1>
          <h4>{detail.genres.map(g => g.name).join(', ')} * {prettyTime(detail.runtime)}</h4>
          </header>
